	...
	, const u8 *mac_addr, struct key_params *params)
{
	...
	if (mac_addr)
		RTW_WARN(FUNC_NDEV_FMT" adding key for %pM\n", FUNC_NDEV_ARG(ndev), mac_addr);
	RTW_WARN(FUNC_NDEV_FMT" cipher=0x%x\n", FUNC_NDEV_ARG(ndev), params->cipher);
	RTW_WARN(FUNC_NDEV_FMT" key_len=%d, key_index=%d\n", FUNC_NDEV_ARG(ndev), params->key_len, key_index);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)) || defined(COMPAT_KERNEL_RELEASE)
	RTW_WARN(FUNC_NDEV_FMT" pairwise=%d\n", FUNC_NDEV_ARG(ndev), pairwise);
#endif
...
)
{
	...
	// RTW_PRINT("del_station lock assoc");

	phead = &pstapriv->asoc_list;
	plist = get_next(phead);

	// if (rtw_end_of_queue_search(phead, plist) == _FALSE) {
	// 	RTW_PRINT(FUNC_ADPT_FMT" assoc_list, cnt:%u\n"
	// 		, FUNC_ADPT_ARG(padapter), pstapriv->asoc_list_cnt);
	// }

	/* check asoc_queue */
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		// RTW_PRINT("del_station iter, phead=%08X, plist=%08X, plist_prev=%08X\n", (u64 *)phead, (u64 *)plist, (u64 *)plist->prev);
	...
	_exit_critical_bh(&pstapriv->asoc_list_lock, &irqL);
	// RTW_PRINT("del_station unlock assoc");
...
void rtw_cfg80211_rx_mframe(_adapter *adapter, union recv_frame *rframe, const char *msg)
{
	...
	RTW_PRINT("RTW_Rx:ch=%d(%d), ta="MAC_FMT"\n", ch, sch, MAC_ARG(get_addr2_ptr(frame)));
...
{
	RTW_WARN("_cfg80211_rtw_mgmt_tx\n");
	struct xmit_frame	*pmgntframe;
	struct pkt_attrib	*pattrib;
	unsigned char	*pframe;
	int ret = _FAIL;
...
{
	RTW_WARN("rtw_cfg80211_preinit_wiphy\n");
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
...
{
	RTW_WARN("rtw_cfg80211_ndev_res_alloc\n");
	int ret = _FAIL;
...
{
	RTW_WARN("rtw_cfg80211_ndev_res_register\n");
	int ret = _FAIL;
