...
u32 upload_txpktbuf_8812au(_adapter *adapter, u8 *buf, u32 buflen)
{
	...
	RTW_PRINT("upload_txpktbuf_8812au");
...
static s32 rtw_dump_xframe(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	// RTW_PRINT("rtw_dump_xframe, pktlen=%d\n", pxmitframe->attrib.pktlen);
...
#ifdef CONFIG_RTW_NETLINK_MONITOR
		nl_server_send_data(RTW_DIR_TX, mem_addr + TXDESC_SIZE, sz, pattrib->injected);
#endif
