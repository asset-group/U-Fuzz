/**************************************************************************************************
  Filename:       eccapi_263.c
  Revised:        $Date: 2014-06-25 18:46:09 -0700 (Wed, 25 Jun 2014) $
  Revision:       $Revision: 39225 $

  Description:    This file implements a dummy version of the the Certicom API.
...
int ZSE_ECDSASign283(unsigned char *privateKey, unsigned char *msgDigest,
              GetRandomDataFunc *GetRandomData,
              unsigned char *r, unsigned char *s,
              YieldFunc *yield, unsigned long yieldLevel )
{
  return ( MCE_ERR_NULL_FUNC_PTR );   // Failure
}

int ZSE_ECDSAVerify283(unsigned char *msgDigest, unsigned char *publicKey,
                unsigned char *r, unsigned char *s,
                YieldFunc *yield, unsigned long yieldLevel )
{
  return ( MCE_ERR_FAIL_VERIFY );   // Failure
}

int ZSE_ECCGenerateKey283(unsigned char *privateKey, unsigned char *publicKey,
                   GetRandomDataFunc *GetRandomData,
                   YieldFunc *yield, unsigned long yieldLevel )
{
  return ( MCE_ERR_NULL_FUNC_PTR );   // Failure
...
{
  return ( MCE_ERR_BAD_INPUT );   // Failure
}
