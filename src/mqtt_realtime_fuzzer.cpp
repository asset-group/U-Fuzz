...
#include "Process.hpp"
#include "drivers/MqttRealTimeDriver.hpp"
...
// this line code means the type of protocol your want to save the capture
...
int main(int argc, char **argv)
{
    ...
    timer_global = loop.onIntervalMS(100000, [&](){
        ...
        std::string msg = "Server Crashed due to timeout";
    ...
    while (true) {
        ...
        case NETLINK_EVT_RX: {
            ...
            if (ignore_packet)
                goto TX_END;
            // this line of code excludes all packets which is not coap 
            ...
            // handle the tcp zero window here
            if (string_contains(packet_summary(),"TCP ZeroWindow")){
                std::string hostname{"127.0.0.1"};
                uint16_t port = 9000;
                int sock = ::socket(AF_INET, SOCK_DGRAM, 0);
                ...
                std::string msg = "Client Crashed due to TCP ZeroWindow";
                int n_bytes = ::sendto(sock, msg.c_str(), msg.length(), 0, reinterpret_cast<sockaddr*>(&destination), sizeof(destination));
                std::cout << n_bytes << " bytes sent" << std::endl;
                ::close(sock);
            }
        ...
        TX_END:
            ...
            if (counter_tx_for_iteration > 100) {
                ...
                // use this time difference function
        ...
        case NETLINK_EVT_TX: {
            ...
            if (counter_destnation_unreachable > 2){
                ...
                std::string msg = "Server Crashed Due to Destination Unreachable";
                int n_bytes = ::sendto(sock, msg.c_str(), msg.length(), 0, reinterpret_cast<sockaddr*>(&destination), sizeof(destination));
                std::cout << n_bytes << " bytes sent" << std::endl;
                ::close(sock);
