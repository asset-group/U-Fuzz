...
private:
    ...
    {
        if (Device != nullptr && Program != nullptr && Filter != nullptr) {
            string cmd_str = "";
            if (Device->size())
                cmd_str += "-s " + *Device + " ";
    ...
    bool run_adb_command(string cmd)
    {
        ...
        if (cmd_str.size()) {
            int res = system(("adb " + cmd_str).c_str());
...
public:
    ...
    bool init()
    {
        if (enable && *enable && Device != nullptr && Program != nullptr && Filter != nullptr)
            return init(*Device, *Program, *Filter);
    ...
    bool init(string &device, string &program, string &filter)
    {
        Device = &device;
        Program = &program;
        Filter = &filter;

        // if (ADBPreCommands && ADBPreCommands->size())
        // {
        //     GL1Y(TAG, "Executing SSH Pre-Commands (Target Setup)");
        //     for (string &cmd : *ADBPreCommands)
        //     {
        //         GL1M(TAG, "--> ", cmd);
