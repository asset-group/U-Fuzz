...
#include "libs/libboost-serialization/boost/archive/binary_iarchive.hpp"
#include "libs/libboost-serialization/boost/archive/binary_oarchive.hpp"
#include "libs/libboost-serialization/boost/iostreams/filter/gzip.hpp"
#include "libs/libboost-serialization/boost/iostreams/filtering_stream.hpp"
#include "libs/libboost-serialization/boost/lexical_cast.hpp"
#include <functional>
#include <iostream>
#include <mutex>
...
private:
    void problem_handler()
    {
        ...
        if (StateMachine.config.fuzzing.restore_session) {
            LoadSession();
        }
...
public:
             ...
             const string session_save_dir = "")
    {
        ...
        if (!StateMachine.config.fuzzing.enable_optimization && !_session_restored) {
            SetDefaultX();
    ...
    vector_double &Iteration(double fitness_value)
    {
        ...
        if (StateMachine.config.fuzzing.save_session) {
    ...
    void SetDefaultX()
    {
        // TODO: distribute other default parameters if optimization is disabled
