...
{
	RTW_PRINT("1. xmitframe_swencrypt\n");
	struct	pkt_attrib	*pattrib = &pxmitframe->attrib;
	/* struct 	security_priv	*psecuritypriv=&padapter->securitypriv; */
...
{
	RTW_WARN("rtw_monitor_xmit_entry\n");
	u16 frame_ctl;
...
//	_rtw_memcpy(pframe, (void *)checking, len);
	...
	// Update ra and ta address
	_rtw_memcpy(pattrib->ra, pwlanhdr->addr1, ETH_ALEN);
	_rtw_memcpy(pattrib->ta, pwlanhdr->addr2, ETH_ALEN);
