...
extern void update_attrib_phy_info(_adapter *padapter, struct pkt_attrib *pattrib, struct sta_info *psta);
...
unsigned int OnAuth(_adapter *padapter, union recv_frame *precv_frame)
{
...
#ifdef CONFIG_IOCTL_CFG80211
	RTW_PRINT("alg=%d, auth_mode=%d\n", algorithm, auth_mode);
	RTW_PRINT("REPORT_MGMT=%d\n",GET_CFG80211_REPORT_MGMT(adapter_wdev_data(padapter), IEEE80211_STYPE_AUTH));
	...
	if (algorithm == 0 && (auth_mode == 0 || auth_mode == 2 || auth_mode == 3)) {
		if (seq == 1) {
			...
			RTW_WARN("(2)auth rejected because out of seq [rx_seq=%d, exp_seq=%d]!\n",
				 seq, pstat->auth_seq + 1);
			status = _STATS_OUT_OF_AUTH_SEQ_;
			goto auth_fail;
		}
		...
		if (seq == 1) {
			...
			RTW_WARN("checking for challenging txt...\n");

			p = rtw_get_ie(pframe + WLAN_HDR_A3_LEN + 4 + _AUTH_IE_OFFSET_ , _CHLGETXT_IE_, (int *)&ie_len,
				len - WLAN_HDR_A3_LEN - _AUTH_IE_OFFSET_ - 4);

			if ((p == NULL) || (ie_len <= 0)) {
				RTW_WARN("auth rejected because challenge failure!(1)\n");
			...
			if (_rtw_memcmp((void *)(p + 2), pstat->chg_txt, 128)) {
				...
				RTW_WARN("auth rejected because challenge failure!\n");
				status = _STATS_CHALLENGE_FAIL_;
				goto auth_fail;
			}
		} else {
			RTW_WARN("(3)auth rejected because out of seq [rx_seq=%d, exp_seq=%d]!\n",
				 seq, pstat->auth_seq + 1);
			status = _STATS_OUT_OF_AUTH_SEQ_;
			goto auth_fail;
		}
...
#ifdef CONFIG_RTW_NETLINK_MONITOR
	// nl_server_send_data(RTW_DIR_TX, (u8 *)(pmgntframe->buf_addr) + TXDESC_OFFSET, pmgntframe->attrib.pktlen);
#endif
...
{
	// RTW_WARN("dump_mgntframe_and_wait\n");
	s32 ret = _FAIL;
	_irqL irqL;
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
...
{
	// RTW_WARN("dump_mgntframe_and_wait_ack_timeout\n");
...
int rtw_sae_preprocess(_adapter *adapter, const u8 *buf, u32 len, u8 tx)
{
	...
	RTW_PRINT("RTW_%s:AUTH alg:0x%04x, seq:0x%04x, status:0x%04x, mesg:%s\n",
