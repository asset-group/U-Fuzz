...
union recv_frame *decryptor(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	RTW_PRINT("prxattrib->bdecrypted:%d\n", prxattrib->bdecrypted);
	if (prxattrib->encrypt && !prxattrib->bdecrypted) {
		RTW_PRINT("2. Decrypting...\n");
...
static inline void dump_packet(u8 *ptr)
{
...
static inline void dump_rx_packet(u8 *ptr)
{
	...
	RTW_WARN("#############################\n");
	for (i = 0; i < 64; i = i + 8)
		RTW_WARN("%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:\n", *(ptr + i),
...
int recv_func_posthandle(_adapter *padapter, union recv_frame *prframe)
{
	...
	// RTW_WARN("recv_func_posthandle\n");
	prframe = decryptor(padapter, prframe);
	dump_packet(prframe->u.hdr.rx_data);
#ifdef CONFIG_RTW_NETLINK_MONITOR
	// nl_server_send_data(RTW_DIR_RX, prframe->u.hdr.rx_data, prframe->u.hdr.len);
#endif
...
#ifdef CONFIG_RTW_NETLINK_MONITOR
	nl_server_send_data(RTW_DIR_RX, rframe->u.hdr.rx_data, rframe->u.hdr.len, 0);
#endif
