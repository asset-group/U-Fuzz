...
void	expire_timeout_chk(_adapter *padapter)
{
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		// RTW_PRINT("expire_timeout iter, phead=%08X, plist=%08X\n, plist_prev=%08x", (u64 *)phead, (u64 *)plist, (u64 *)plist->prev);
	...
	_exit_critical_bh(&pstapriv->auth_list_lock, &irqL);
	// RTW_PRINT("Exit expire_timeout auth_list\n");
	psta = NULL;
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		...
		plist = get_next(plist);
		// RTW_PRINT("expire_timeout iter, phead=%08X, plist=%08X, plist_prev=%08X\n", (u64 *)phead, (u64 *)plist, (u64 *)plist->prev);
		// yield();
		// if(plist->next == plist)
		// 	plist->next = phead;
...
#define KEEP_ALIVE_TRYCNT (3)
	...
	// RTW_PRINT("Exit expire_timeout assoc_list\n");
	if (chk_alive_num) {
	...
	// RTW_PRINT("Exit whole expire_timeout\n");
}
...
update_beacon:
	...
	if (mlme_act != MLME_OPCH_SWITCH
		...
		if (rtw_is_list_empty(&padapter->list)) {
			...
			pdvobj->nr_ap_if++;
			pdvobj->inter_bcn_space = padapter->registrypriv.default_beacon_interval / pdvobj->nr_ap_if;
		}
...
void stop_ap_mode(_adapter *padapter)
{
	...
	if (pmlmeext->bstart_bss == _TRUE) {
		...
		if (pdvobj->nr_ap_if > 0)
			...
			pdvobj->inter_bcn_space = padapter->registrypriv.default_beacon_interval;
		#ifdef CONFIG_FW_HANDLE_TXBCN
...
#define BEACON_EARLY_TIME		20	/* unit:TU*/
	...
	bcn_interval_us = padapter->registrypriv.default_beacon_interval * NET80211_TU_TO_US;
