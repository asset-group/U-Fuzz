...
{
	static _list *last_head = NULL;
	static _list *last_plist = NULL;
	static u32 same_list_cnt = 0;

	// Race Condition Workaround: detect and fix list serach hangs
	if (head==last_head && plist==last_plist)
	{
		if(++same_list_cnt >= 100){
			plist->next=head;
			plist->prev=head;
			same_list_cnt=0;
			RTW_PRINT("List hang detected\n");
		}
	}
	else
		same_list_cnt=0;	
