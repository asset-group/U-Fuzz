...
int rtw_default_beacon_interval = DEFAULT_BCN_INTERVAL;
module_param(rtw_default_beacon_interval, int, 0644);
MODULE_PARM_DESC(rtw_default_beacon_interval, "Default beacon frame interval in ms");
...
uint rtw_drv_log_level = RTW_LOG_LEVEL;
...
uint loadparam(_adapter *padapter)
{
	...
	if(rtw_default_beacon_interval < 100)
	{	
		RTW_WARN("Beacon interval of %d invalid, defaulting to 100ms\n");
		rtw_default_beacon_interval = 100;
	}

	registry_par->default_beacon_interval = rtw_default_beacon_interval;
...
int rtw_ndev_init(struct net_device *dev)
{
	...
	RTW_WARN(FUNC_ADPT_FMT" if%d mac_addr="MAC_FMT"\n"
...
struct dvobj_priv *devobj_init(void)
{
	...
	pdvobj->nr_ap_if = 0;
	pdvobj->inter_bcn_space = rtw_default_beacon_interval; /* default value is equal to the default beacon_interval (100ms) */
