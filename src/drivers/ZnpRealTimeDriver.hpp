...
#ifndef __ZnpRealTimeDriver__
#define __ZnpRealTimeDriver__
...
public:
    ...
    bool init(Config &config)
    {
        ...
        try {
            ...
            this->uart = new Serial(config.bluetooth.serial_port,
                                    ...
                                    Timeout::simpleTimeout(50), // Read timeout
        ...
        if (this->isOpen) {
            ...
            // }
            // else {
            //     GL1R(TAG, "Could not set own BDADDR to ", config.bluetooth.ow-n_bd_address);
            //     this->ready = false;-
            //     this->close();
            //     return false;
            // }
    ...
    void hci_bridge_write(uint8_t *buf, size_t buf_len)
    {
        if (this->enable_bridge_hci && this->pty_master) {
            ...
            if (this->enable_uart_debug_hci)
                LOG3("[ZNP] ", "ZNP FROM ESP32:", strtk::convert_bin_to_hex(string((char *)buf, buf_len)));
        }
    }
        ...
        if (en && !this->enable_bridge_hci) {
            if (openpty(&pty_master, &pty_slave, pty_name, NULL, NULL) == 0) {
                ...
                if (tcsetattr(this->pty_master, TCSANOW, &params_master) < 0) {
                    ...
                    hci_bridge_thread = new std::thread(&ZnpRealTimeDriver::hci_bridge_read, this);
                    pthread_setname_np(hci_bridge_thread->native_handle(), "hci_bridge");
                    GL1G(TAG, "HCI Bridge ON: ", this->hci_bridge_name);
                    system("sudo rm -r /dev/ttyV0");
                    system(("ln -snf "+this->hci_bridge_name+" /dev/ttyV0").c_str());
    ...
    driver_event receive()
    {
        ...
        while (this->ready) {
            ...
            // hci_bridge_write()
            hci_size = uart->read(hci_buff, 256);
            if (hci_size > 0) {
                hci_bridge_write(hci_buff, hci_size);
                std::cout << "this is the hci_size " << hci_size << std::endl;
            }
            //     hci_size = 1;
            //     evt = cmd;
