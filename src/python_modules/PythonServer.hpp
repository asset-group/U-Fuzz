...
private:
    ...
    static void enable_idle_scheduler()
    {
        ...
        int ret = sched_setscheduler(0, SCHED_IDLE, &sp);
    }

public:
    const char *TAG = "[PythonServer] ";
    bool started = false;
    ...
    {
        _conf = &conf;
        enable_events = conf.server_options.enable_events;

        if (conf.server_options.server_module >= conf.server_options.server_modules_list.size()) {
            LOG2R(TAG, "Module in Modules List out of range!");
            return false;
        }

        return init(conf.server_options.listen_address, conf.server_options.port,
                    conf.server_options.logging,
                    conf.server_options.server_modules_list[conf.server_options.server_module],
                    conf.server_options.api_namespace, exclude_core_);
    }
              ...
              int exclude_core_ = -1)
    {
        ...
        try {
            ...
            server_thread = new thread([&]() {
                // Set schedule to IDLE (low priority)
